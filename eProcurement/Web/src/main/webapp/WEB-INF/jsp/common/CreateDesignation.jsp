...
<title>${createdesignation}</title>
					...
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">				 	
								<div class="box">
									...
									<div class="box-header with-border">
										<c:choose>
											<c:when test="${optType eq 'edit'}">
												<spring:message code="lbl_edit_designation"
													var="editdesignation" />
									...
									<div class="box-body">
										<div class="row">
											<c:choose>
												<c:when test="${optType eq 'edit'}">
													<spring:url value="/common/user/editDesignation"
														var="submitDesignation" />
												</c:when>
												<c:otherwise>
													<spring:url value="/common/user/addDesignation"
														var="submitDesignation" />
												</c:otherwise>
											</c:choose>
												...
												<div class="row">
													<form:form action="${submitDesignation}" onsubmit="return validate();" name="frmdesignation"  method="post" modelAttribute="tblDesignation">
														<div class="col-md-6">
		                                              		...
		                                              		<div class="fr-grp">
		                                                    	<label class="lblfr-fields"><spring:message code="lbl_tenderauthority" />: </label>
																<c:if test="${sessionObject.isCTPLUser eq 1}">
																	<c:choose>
																		...
																		<c:otherwise>
																			<select class="form-control fr-cntrl" name="DeptId" id="DeptId"
																				isrequired="true"
																				onblur="javascript:{if(validateCombo(this)){}}"
																				title="Organization">
															...
															<div class="fr-grp">
		                                                    	<label class="lblfr-fields">Designation name</label>
																<form:input class="form-control fr-cntrl" id="designationName" path="designationName" validarr="required@@tenderbrief:50"
																tovalid="true" onkeypress="validateTextComponent(this)" onblur="validateTextComponent(this)" title="designation name"></form:input>
													...
													<!-- 									</div> -->
															...
															<c:choose>
																<c:when test="${optType eq 'edit'}">
																	<button type="submit" id="addDesignation" class="btn btn-submit">Update</button>
																</c:when>
																<c:otherwise>
								...
								<c:if test="${sessionObject.isCTPLUser eq 0}">
									<input type="hidden" value="${tblGrandParentDept.deptId}"
										class="searchEqual" columnName="b.deptId" name="b.deptId">
	...
	<script type="text/javascript">
		...
		function callActionItem(cthis) {
			...
			if (actionname.toLowerCase() == "edit") {
				...
				window.location = "${pageContext.servletContext.contextPath}/common/user/geteditdesignation/"
						+ deptId;
			}
		...
		function searchAjax() {
			var data = {};
			var searchValue = $("#designationName").val() + '@@'
					+ $("#DeptId").val() + '@@0@@0';
			...
			$
					.ajax({
						...
						url : "${pageContext.servletContext.contextPath}/common/user/isdesignationexists/"
								+ searchValue + "/" + keyword,
						data : data,
						timeout : 100000,
			...
			$
					.ajax({
						...
						success : function(data) {
							...
							$('select[id="deptId"] option:selected').attr(
									"selected", null);
							$('select[id="deptId"] option[value="-1"]').attr(
									"selected", "selected");
							$('select[id="subDept"] option:selected').attr(
									"selected", null);
							$('select[id="subDept"] option[value="-1"]').attr(
									"selected", "selected");
							if (mode == 1) {
								if (parentDeptId > 0) {
	...
	<script
	src="${pageContext.servletContext.contextPath}/resources/js/blockUI.js"
	type="text/javascript"></script>
<script
	src="${pageContext.request.contextPath}/resources/js/commonListing.js"></script>
