...
<div class="box-body">
		<div class="row" id="viewComparativeRptId">
                                                ...
                                                <tr>
                                                	<c:forEach items="${lstColumnDtl}" var="obColumn">
                                                		<c:if test="${obColumn.tableid eq obTable.tableId}">
                                                            ...
                                                            <c:if test="${obColumn.filledBy ne 1 and obColumn.isShown ne 0}">
                                                                <c:set var="fillOtherCount" value="${fillOtherCount+1}" />
                                                            </c:if>
                                                		</c:if>
                                                	</c:forEach>
                                                </tr>
                                                <c:set var="cmpNameCount" value="${fn:length(lstCompanyDtl)}" />
                                                <c:forEach begin="0" end="${obTable.noOfRows-1}" var="rowId">
                                                	...
                                                	<tr>
                                                        ...
                                                        <c:forEach items="${lstColumnDtl}" var="obColumn">
                                                            ...
                                                            <c:forEach items="${lstCellDtl}" var="obCellDtl">
                                                                <c:if test="${obCellDtl.rowId eq rowId && obCellDtl.tableId eq obTable.tableId && obColumn.columnId eq obCellDtl.columnId}">
                                                                    <c:choose>
                                                                    	<c:when test="${obColumn.filledBy eq 1}">
                                                                            <td class="a-left">${empty obCellDtl.cellValue ? '-' : obCellDtl.cellValue}</td>
                                                                            <c:if test="${empty obCellDtl.cellValue}"><c:set var="isLastRow" value="true" /></c:if>
                                                                        </c:when>
                                                                        <c:otherwise>
                                                                        	<c:if test="${fillByCount eq 0}">
                                                                        		<c:choose>
                                                                            	<c:when test="${isGrandTotalDone eq 'false' && isLastRow eq 'true'}">
                                                                            	...
                                                                            	<c:otherwise>
                                                                                    ...
                                                                                    <table  class="table">
                                                                                        ...
                                                                                        <tr>
                                                                                            ...
                                                                                            <c:if test="${obColumn.tableid eq obTable.tableId and companyName ne ''}">
                                                                                                    <th class="a-left" width="40%">${lblBidderName}</th>
                                                                                                    <c:set var="companyName" value=""/>
                                                                                            </c:if>
                                                                                            <c:forEach items="${lstColumnDtl}" var="obColumn1">
                                                                                                <c:if test="${obColumn1.filledBy ne 1 and obColumn1.tableid eq obTable.tableId}">
                                                                                                	<c:if test="${obColumn1.isShown ne 0}">
                                                                                                    	<th class="a-left" width="${60 / fillOtherCount}%">${obColumn1.columnHeader}
                                                                                                    	<c:if test="${obColumn1.filledBy eq 3 and tblTender.isRebateApplicable eq 1 and obColumn1.isGTColumn eq 1}">
                                                                                                    		(without rebate)
                                                                                                    	</c:if>
                                                                                                    	</th>
                                                                                                    </c:if>
                                                                                                </c:if>
                                                                                                <c:if test="${obColumn1.formula ne null && fn:startsWith(obColumn1.formula,'TOTAL')}">
	                                                                                                <c:set var="isTotExist" value="1"/>
	                                                                                            </c:if>
                                                                                            </c:forEach>
                                                                                       </tr>
...
<%--                                                                                                 <c:otherwise> --%>
                                                                                                    <c:forEach items="${lstCompanyDtl}" var="obCompany" begin="0" end="${cmpNameCount}">
                                                                                                            ...
                                                                                                            <tr class="comparativeRptTR">
                                                                                                                <td>${obCompany[5]}
                                                                                                                	<c:if test="${obForm.isPriceBid eq 1 and tblTender.biddingType eq 2}">
																		                                        		<label><b>(${obCompany[7]}) </b></label>
																		                                        	</c:if>
                                                                                                                </td>
                                                                                                            
                                                                                                            
                                                                                                                <c:forEach items="${lstColumnDtl}" var="obColumn">
                                                                                                                     <c:if test="${obColumn.tableid eq obTable.tableId and obColumn.filledBy ne 1}">
                                                                                                                    	 <c:if test="${obColumn.isShown ne 0}">
                                                                                                                             <c:set var="fetchCol" value="${obCompany[1]}_${obTable.tableId}_${obColumn.columnId}_${rowId}" />
                                                                                                                                 <c:choose>
                                                                                                                                     <c:when test="${(obTable.noOfRows eq rowId) or not empty mapBidData[fetchCol]}">
                ...
                <div class="row"> 
                    ...
                    <tr class="border-bottom-none">
                        <td class="a-left no-padding">
                            <input type="button" class="btn noExport" onclick="exportContent('viewComparativeRptId','ComparativeReport${tenderId}',0)" value="PDF">
							<input type="button" class="btn noExport" onclick="exportContent('viewComparativeRptId','ComparativeReport${tenderId}',5)" value="Print">
                            <%-- <c:choose>
		                        <c:when test="${userTypeId eq 2}">
		                             <a href="${pageContext.servletContext.contextPath}/etender/bidder/biddingtenderdashboardcontent/${tenderId}/7"><< ${backDashboard}</a>
		                        </c:when>
